...
pnpm build        # Vite build, then strips model files from output
...
- **Deployment**: Cloudflare Workers (`src/server.ts` entry) + R2 for model storage
...
Models are too large to ship in the build. The post-build script `scripts/strip-models-from-build.mjs` removes them from the Vite output. In production they are served from Cloudflare R2 via the `/models/` Worker proxy (`src/server/serve-models.ts`). Locally, models live in `public/models/`.

`checkModelCached()` uses a HEAD fetch in dev (to check the filesystem) and a CacheStorage scan in prod (R2 always returns 200 so a live fetch would never reflect the user's download state).
