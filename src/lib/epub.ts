...
// Project Gutenberg's EbookMaker tool decorates every illustration with a
// tiny "click to enlarge" icon — an <img> wrapped in an <a title="linked
// image"> pointing at the (now-dropped) duplicate page. It sits right next
// to the real illustration and would otherwise be extracted as if it were
// the picture itself.
function isEbookmakerZoomIcon(img: Element): boolean {
  const title = img.closest("a")?.getAttribute("title")?.trim().toLowerCase()
  return title === "linked image"
}

async function imageAssetFromImg(
  img: Element,
  xhtmlPath: string,
  zip: JSZip
  ...
  return { id: resolved, mime_type: guessMimeType(resolved), data_base64: b64 }
}
...
async function extractImages(
  ...
  for (const img of Array.from(el.querySelectorAll("img"))) {
    if (isEbookmakerZoomIcon(img)) continue
    const src = img.getAttribute("src")
...
// Project Gutenberg's EbookMaker tool splits every inline illustration into
// a second, standalone "linked image" page appended after the main text
// (an image + a "back" link to the inline occurrence) — a print-oriented
// duplicate meant for e-readers that page through the image separately, not
// a real reading-order paragraph. Its <title> is always exactly this literal
// string, so we can drop the whole page rather than let the duplicate image
// show up a second time with no accompanying sentence to align against.
function isEbookmakerLinkedImagePage(doc: Document): boolean {
  const title = doc.querySelector("title")?.textContent?.trim() ?? ""
  return /^["“”']?linked image["“”']?$/i.test(title)
}
...
async function parseXhtmlBlocks(
  ...
  if (isEbookmakerLinkedImagePage(doc)) return []
  ...
  // Priority 2: <p> tags, plus any illustration that lives outside every
  // <p> — some EPUBs (notably Gutenberg's EbookMaker output) float the real
  // image directly inside a wrapping <div>, with only a decorative
  // "enlarge" icon actually sitting inside a <p>. Merge both element sets
  // back into document order so images land next to their real position.
  const pTags = Array.from(doc.querySelectorAll("p"))
  if (pTags.length > 0) {
    const looseImgs = Array.from(doc.querySelectorAll("img")).filter(
      (img) => !img.closest("p") && !isEbookmakerZoomIcon(img)
    )
    const blockEls: Element[] = [...pTags, ...looseImgs].sort((a, b) => {
    ...
    const results = await Promise.all(
      blockEls.map(async (el) => {
        if (el.tagName.toLowerCase() === "img") {
          const asset = await imageAssetFromImg(el, xhtmlPath, zip)
          return { text: "", images: asset ? [asset] : [] }
        }
        return toBlock(el)
      })
    )
