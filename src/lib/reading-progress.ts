...
/**
 * Per-alignment view preferences (view mode + page number visibility).
 *
 * Same two-layer pattern as reading progress: the URL search params are the
 * session/shareable layer, this localStorage entry (pt:viewprefs:{id}) is the
 * persistent fallback used when a param isn't present in the URL. Kept as a
 * separate key from progressKey() so it doesn't get gated behind the
 * charCount/totalChars validity check, and doesn't affect book progress
 * (which shares the progress storage via the "book:" prefix but has no
 * view/pageNumHidden concept).
 */
export interface AlignmentViewPrefs {
  view: "side-by-side" | "popover"
  pageNumHidden: boolean
}

const viewPrefsKey = (id: string) => `pt:viewprefs:${id}`

export function getAlignmentViewPrefs(
  id: string
): Partial<AlignmentViewPrefs> | null {
  try {
    const raw = localStorage.getItem(viewPrefsKey(id))
    if (!raw) return null
    const p = JSON.parse(raw)
    const result: Partial<AlignmentViewPrefs> = {}
    if (p.view === "side-by-side" || p.view === "popover") result.view = p.view
    if (typeof p.pageNumHidden === "boolean")
      result.pageNumHidden = p.pageNumHidden
...
export function setAlignmentViewPrefs(
  id: string,
  prefs: Partial<AlignmentViewPrefs>
): void {
  try {
    const existing = getAlignmentViewPrefs(id) ?? {}
    localStorage.setItem(
      viewPrefsKey(id),
